...
fn render_config_toml(cfg: &config::Config, claude: bool) -> String {
    let mut out = String::from("# redan.toml -- generated by `redan init`\n");
    out.push_str("# See: https://github.com/getredan/redan\n\n");

    if let Some(ref image) = cfg.image {
        out.push_str(&format!("image = \"{image}\"\n"));
    }
    if let Some(ref command) = cfg.command {
        if claude {
            out.push_str("# Remove --dangerously-skip-permissions to re-enable Claude's\n");
            out.push_str("# built-in permission prompts inside the VM.\n");
        }
        out.push_str(&format!("command = \"{command}\"\n"));
    }
    if let Some(interactive) = cfg.interactive {
    ...
    if !cfg.network.allow.is_empty() {
        out.push_str("\n[network]\n");
        if claude {
            out.push_str("# Hosts Claude Code needs to function. Add project-specific\n");
            out.push_str("# API hosts here. Remove any you don't need.\n");
        } else {
            out.push_str("# Allowed outbound hosts. Empty = deny all, [\"*\"] = allow all.\n");
        }
        out.push_str("allow = [\n");
    ...
    if !cfg.env.is_empty() {
        out.push_str("\n[env]\n");
        if claude {
            out.push_str("# CLAUDE_CONFIG_DIR in the workspace mount persists sessions\n");
            out.push_str("# across redan restarts (conversation history, preferences).\n");
        }
        for (k, v) in &cfg.env {
...
fn init(claude: bool) {
    ...
    let content = render_config_toml(&cfg, claude);
